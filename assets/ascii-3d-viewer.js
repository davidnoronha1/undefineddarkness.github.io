// glb-model-viewer.js
import * as THREE from "https://cdn.jsdelivr.net/npm/three@0.155.0/+esm";
import { GLTFLoader } from "https://cdn.jsdelivr.net/npm/three@0.155.0/examples/jsm/loaders/GLTFLoader.js/+esm";
import { DRACOLoader } from "https://cdn.jsdelivr.net/npm/three@0.155.0/examples/jsm/loaders/DRACOLoader.js/+esm";
...
export class GlbModelViewer extends HTMLElement {
  constructor() {
    ...
    this.wireframeColor = new THREE.Color(0xffffff);
  }
  ...
  parseAttributes() {
    this.scaleFactor = parseFloat(this.getAttribute("scale")) || 1;
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    if ((name === "wireframe" || name === "wireframe-color") && this.pivotGroup) {
      this.wireframe = this.hasAttribute("wireframe");
      if (name === "wireframe-color") {
        try {
          this.wireframeColor.set(newValue || 0xffffff);
        } catch {}
      }
  ...
  updateWireframe() {
    ...
    this.pivotGroup.traverse((obj) => {
      ...
      const existing = this._wireframeOverlays.get(obj);
      ...
      if (this.wireframe) {
        if (!existing) {
          ...
          wire.renderOrder = (obj.renderOrder || 0) + 1;
          wire.frustumCulled = false;
          obj.add(wire);
          this._wireframeOverlays.set(obj, { wire, geo: wfGeo, mat: wfMat });
        } else {
        ...
        if (existing) {
          if (existing.wire.parent) existing.wire.parent.remove(existing.wire);
          existing.geo.dispose();
          existing.mat.dispose();
          this._wireframeOverlays.delete(obj);
        }
      }
    });
  }
  ...
  _disposeAllWireframes() {
    this._wireframeOverlays.forEach((ov) => {
      if (ov.wire.parent) ov.wire.parent.remove(ov.wire);
      try {
        ov.geo.dispose();
      } catch {}
      try {
  ...
  addInteractions() {
    ...
    window.addEventListener("mousemove", (e) => {
      if (!this.isDragging || !this.pivotGroup) return;
      ...
      if (!this.disablePan && e.shiftKey) {
        ...
        const deltaRotationQuaternion = new THREE.Quaternion().setFromEuler(
          new THREE.Euler(dy * rotateSpeed, dx * rotateSpeed, 0, "XYZ")
        );
        this.targetRotation.multiplyQuaternions(
    ...
    canvas.addEventListener("wheel", (e) => {
      ...
      this.targetScale = Math.max(
        0.1,
        Math.min(5, this.targetScale * (1 + zoomDelta))
      );
    });
  }
  ...
  animate() {
    ...
    if (this.pivotGroup) {
      ...
      this.pivotGroup.setRotationFromQuaternion(this.rotation);
      this.scaleFactor = THREE.MathUtils.lerp(
        this.scaleFactor,
        this.targetScale,
        lerpFactor
      );
